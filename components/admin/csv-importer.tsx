  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (data.inserted > 0 || data.updated > 0) {
        toast.success(`Imported ${data.inserted + data.updated} podcasts`)
      }
      if (data.errors?.length) {
        toast.error(`${data.errors.length} errors — see details below`)
      }
    } catch {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      {result && (
        <div className="card p-5 space-y-3">
          <p className="text-sm font-semibold text-stone mb-3">Import results</p>
          <div className="flex items-center gap-2 text-sm">
            <CheckCircle size={16} className="text-emerald-400" />
            <span className="text-stone">{result.inserted + result.updated} podcasts imported successfully</span>
          </div>
          {result.errors.length > 0 && (
            <div className="space-y-1.5">
              <div className="flex items-center gap-2 text-sm">
                <XCircle size={16} className="text-crimson" />
                <span className="text-stone">{result.errors.length} errors</span>
              </div>
